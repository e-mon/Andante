...
class SphereMenu:UIView, UICollisionBehaviorDelegate{
    ...
    let kAngleOffset:CGFloat = CGFloat(M_PI_2) / 1.8 //2
    let kSphereLength:CGFloat = 80 //80
    let kSphereDamping:Float = 1
    ...
    func commonSetup()
    {
        ...
        self.itemBehavior?.elasticity = 0.25; //0.25
        self.itemBehavior?.density = 0.5;
        self.itemBehavior?.angularResistance = 4; //4
    ...
    func centerForSphereAtIndex(index:Int) -> CGPoint{
        let firstAngle:CGFloat = -20 + CGFloat(M_PI) + (CGFloat(M_PI_2) - kAngleOffset) + CGFloat(index) * kAngleOffset
